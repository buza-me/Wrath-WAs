...
function Context:GetInternalConfig()
  return env.stored or WeakAurasSaved["displays"][env.id]["config"]
end
...
function Context:GetWaitTimeOffset()
  ...
  if env.config.useFrametime then
  ...
  if env.config.isReverseOffset then
    offset = -(offset + frameTime)
  end
...
function Context:SpellQueue(macroSpellIdentifier, unitID)
  ...
  local _, spellName, rankText, icon, castTime, minRange, maxRange, spellID

  if isTBCC then
    spellName, rankText, icon, castTime, minRange, maxRange, spellID = GetSpellInfo(macroSpellIdentifier)
  else
    spellName, rankText, icon, _, _, _, castTime, minRange, maxRange = GetSpellInfo(macroSpellIdentifier)
  end
  ...
  if spellID then
    spellRecord = self:GetSpellRecord(nil, nil, spellID)
  else
    spellRecord = self:GetSpellRecord(spellName, rankText)
  end
  ...
  if isTimePrecise then
    ...
    while ((GetTimePreciseSec() - start) * 1000) < waitFor do
      -- nothing
    end
  else
